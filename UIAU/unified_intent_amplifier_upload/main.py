...
def main():
    ...
    # ── AI-Gated Speak Function ─────────────────────────────────────────────
    def _ai_speak(text: str):
        """Only speak if AI Assist is enabled"""
        if cognitive_engine.get_enabled():
            audio_engine.speak(text)
        else:
            logger.debug("AI speech blocked - AI Assist is OFF: %s", text[:50])
    
    def _ai_tip(text: str):
        """Show tip regardless, but speak only if AI Assist is enabled"""
        overlay.show_tip(text)
    ...
    screen_agent.on_tip = _ai_tip
    screen_agent.on_speak = _ai_speak
    screen_agent.start()
    ...
    # ── Multi-Agent Team ───────────────────────────────────────────────────────
    agent_team.on_tip = _ai_tip
    agent_team.on_speak = _ai_speak
    agent_team.ui_automation = ui_automation
    agent_team.set_screen_agent(screen_agent)

    # ── AI Assistant ─────────────────────────────────────────────────────────
    ai_assistant.on_tip   = _ai_tip
    ai_assistant.on_speak = _ai_speak
    ai_assistant.start()

    # ── Wire cognitive engine to overlay and audio ───────────────────────────
    # ── Cognitive Engine ────────────────────────────────────────────────────
    cognitive_engine.on_tip_callback   = _ai_tip
    cognitive_engine.on_speak_callback = _ai_speak
    ...
    def _toggle_cognitive(enabled: bool):
        cognitive_engine.set_enabled(enabled)
    
    overlay.on_cognitive   = _toggle_cognitive
    overlay.on_read_screen = cognitive_engine.read_screen_now
    ...
    def _on_ideas():
        # Only get ideas if AI Assist is enabled
        if cognitive_engine.get_enabled():
            agent_team.run_async("I am not sure what to do next. Give me 3 ideas.")
        else:
            overlay.show_tip("🧠 AI Assist is OFF - Enable it to get ideas")
            audio_engine.speak("AI Assist is disabled. Enable it in the overlay to get ideas.")
    
    overlay.on_ideas = _on_ideas
    ...
    voice_nav.on_tip           = _ai_tip
    voice_nav.on_speak         = _ai_speak

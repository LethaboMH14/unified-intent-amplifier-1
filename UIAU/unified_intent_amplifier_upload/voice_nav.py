...
class VoiceNavEngine:
    ...
    def _handle_command(self, text: str):
        ...
        # ── Unknown — ask GPT-4o like ChatGPT/Claude ────────────────────────
            ...
            if self.cognitive_engine and hasattr(self.cognitive_engine, "answer_question"):
                self.cognitive_engine.answer_question(text)
            elif self.ai_assistant:
                threading.Thread(
                    target=self.ai_assistant.ask_question,
                    args=(text,), daemon=True
                ).start()
            else:
                if self.on_speak:
                    self.on_speak("I heard you but cannot connect to AI right now.")
